...
function AdminPanel({cu,allUsers,setAllUsers,prizes,setPrizes,shop,notifs,setNotifs,toast,reloadUsers}){
  ...
  const createUser=async()=>{if(!form.name.trim()||!form.password.trim()){toast("Completa nombre y contrasena");return;}if(allUsers.find(u=>u.name.toLowerCase()===form.name.toLowerCase())){toast("Error: ese nombre ya existe");return;}setLoading(true);try{await db.createUser({username:form.name.trim(),full_name:form.name.trim(),password_hash:form.password,role:form.role==="user"?"usuario":form.role,team:form.project,is_active:true,level:1});await reloadUsers();setForm(blank);toast(`Usuario ${form.name} creado`);}catch(e){toast("Error al crear usuario: "+e.message);}setLoading(false);};
  ...
  const tabs=[{id:"users",label:"Usuarios"},{id:"kudos",label:"Dar Kudo"},{id:"notifSend",label:"Enviar Aviso"},{id:"prizes",label:"Premios"}];
  return(
...
export default function App(){
  ...
  // ── AGENTS APP ──
  const userNav=[{id:"dashboard",icon:"🏠",label:"Inicio"},{id:"riddle",icon:"🧠",label:"Riddle"},{id:"task",icon:"📋",label:"Task"},{id:"leaderboard",icon:"🏆",label:"Ranking"},{id:"rewards",icon:"🎁",label:"Premios"},{id:"info",icon:"📖",label:"Como"},{id:"notifs",icon:"🔔",label:"Avisos",badge:unread},{id:"profile",icon:"🎨",label:"Perfil"}];
