...
function StaffKudos({user,allStaff,kudos,onSendKudo,onApproveKudo,isManager,allAgents}){const [tab,setTab]=useState("received");const [sendTab,setSendTab]=useState("staff");const [form,setForm]=useState({toId:"",type:"regular",reason:""});const [sending,setSending]=useState(false);const received=(kudos||[]).filter(k=>k.status==="approved");const pending=(kudos||[]).filter(k=>k.status==="pending");const peers=allStaff.filter(u=>u.active&&u.id!==user.id);const inp={width:"100%",border:`1px solid ${S.border}`,borderRadius:8,padding:"9px 11px",fontSize:13,outline:"none",fontFamily:"inherit",boxSizing:"border-box",background:S.bg,color:S.text};const send=async()=>{if(!form.toId||!form.reason.trim())return;setSending(true);await onSendKudo({recipient_id:form.toId,given_by:user.id,kudo_type:form.type,reason:form.reason,points_awarded:form.type==="gold"?5:1,status:"pending"});setForm({toId:"",type:"regular",reason:""});setSending(false);};const tabs=[{id:"received",label:"Received",show:true},{id:"send",label:"Send Kudo",show:true},{id:"pending",label:`Pending (${pending.length})`,show:isManager}];return(<div style={{paddingBottom:100,background:S.bg,minHeight:"100vh"}}><SCard style={{marginBottom:14,background:`linear-gradient(135deg,${S.accentDk},${S.purple})`,border:"none"}}><div style={{fontSize:32}}>👏</div><div style={{color:S.text,fontWeight:800,fontSize:18}}>Kudos</div><div style={{display:"flex",gap:16,marginTop:10}}><div style={{textAlign:"center"}}><div style={{color:S.yellow,fontWeight:900,fontSize:20}}>{received.filter(k=>k.kudo_type==="gold").length}</div><div style={{color:S.muted,fontSize:10}}>GOLD</div></div><div style={{textAlign:"center"}}><div style={{color:S.accent,fontWeight:900,fontSize:20}}>{received.filter(k=>k.kudo_type==="regular").length}</div><div style={{color:S.muted,fontSize:10}}>REGULAR</div></div></div></SCard><div style={{display:"flex",gap:6,marginBottom:14,overflowX:"auto"}}>{tabs.filter(t=>t.show).map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"7px 14px",borderRadius:9,border:`1px solid ${tab===t.id?S.accent:S.border}`,background:tab===t.id?`${S.accent}22`:S.bgCard,color:tab===t.id?S.accent:S.muted,fontWeight:700,fontSize:12,cursor:"pointer",whiteSpace:"nowrap",fontFamily:"inherit"}}>{t.label}</button>)}</div>{tab==="received"&&(<div>{received.length===0&&<SCard style={{textAlign:"center",padding:32}}><div style={{fontSize:40,marginBottom:8}}>👏</div><div style={{color:S.muted}}>No kudos yet.</div></SCard>}{received.map((k,i)=>(<SCard key={i} style={{marginBottom:10}}><div style={{display:"flex",gap:10,alignItems:"center"}}><div style={{fontSize:28}}>{k.kudo_type==="gold"?"🌟":"👏"}</div><div style={{flex:1}}><div style={{color:k.kudo_type==="gold"?S.yellow:S.accent,fontWeight:700}}>{k.kudo_type==="gold"?"Gold Kudo":"Kudo"} · +{k.points_awarded} pts</div><div style={{color:S.muted,fontSize:12,fontStyle:"italic",marginTop:2}}>"{k.reason}"</div></div></div></SCard>))}</div>)}{tab==="send"&&(<SCard>
        ...
        <div style={{display:"flex",gap:6,marginBottom:12}}>
          {[{id:"staff",label:"⚡ Staff"},{id:"agents",label:"🏆 Agents"}].map(t=>(
            <button key={t.id} onClick={()=>{setSendTab(t.id);setForm(p=>({...p,toId:""}));}} style={{padding:"6px 14px",borderRadius:8,border:`1px solid ${sendTab===t.id?S.accent:S.border}`,background:sendTab===t.id?`${S.accent}22`:S.bg,color:sendTab===t.id?"#a5b4fc":S.muted,fontWeight:700,fontSize:11,cursor:"pointer",fontFamily:"inherit"}}>{t.label}</button>
          ))}
        </div>
        <div style={{marginBottom:12}}><div style={{color:S.muted,fontSize:11,marginBottom:4}}>TO</div>
        <select value={form.toId} onChange={e=>setForm(p=>({...p,toId:e.target.value}))} style={inp}>
          <option value="">Select...</option>
          {sendTab==="staff"?peers.map(u=><option key={u.id} value={u.id}>{u.gameId||u.name} · {STAFF_ROLES[u.role]}</option>):(allAgents||[]).filter(u=>u.active&&u.role==="user").map(u=><option key={u.id} value={u.id}>{u.name} · {u.project}</option>)}
        </select></div><div style={{marginBottom:12}}><div style={{display:"flex",gap:8}}><div onClick={()=>setForm(p=>({...p,type:"regular"}))} style={{flex:1,padding:10,borderRadius:9,border:`1.5px solid ${form.type==="regular"?S.accent:S.border}`,background:form.type==="regular"?`${S.accent}18`:S.bg,cursor:"pointer",textAlign:"center"}}><div style={{fontSize:22}}>👏</div><div style={{color:S.text,fontWeight:700,fontSize:12}}>Kudo (+1pt)</div></div><div onClick={()=>setForm(p=>({...p,type:"gold"}))} style={{flex:1,padding:10,borderRadius:9,border:`1.5px solid ${form.type==="gold"?S.yellow:S.border}`,background:form.type==="gold"?`${S.yellow}18`:S.bg,cursor:"pointer",textAlign:"center"}}><div style={{fontSize:22}}>🌟</div><div style={{color:S.text,fontWeight:700,fontSize:12}}>Gold (+5pts)</div></div></div></div><div style={{marginBottom:14}}><div style={{color:S.muted,fontSize:11,marginBottom:4}}>REASON</div><textarea value={form.reason} onChange={e=>setForm(p=>({...p,reason:e.target.value}))} rows={3} style={{...inp,resize:"vertical"}} placeholder="Why do they deserve this?"/></div><SBtn onClick={async()=>{
          if(sendTab==="agents"){
            if(!form.toId||!form.reason.trim())return;
            const target=(allAgents||[]).find(u=>u.id===form.toId);
            if(!target)return;
            try{
              const pts=form.type==="gold"?5:1;
              await sbFetch("kudos_log",{method:"POST",prefer:"return=representation",body:JSON.stringify({from_user_id:user.id,to_user_id:target.id,reason:form.reason,category:"general",points_given:pts})});
              await sbFetch(`profiles?id=eq.${target.id}`,{method:"PATCH",prefer:"return=representation",body:JSON.stringify({kudos:(target.kudos||0)+(form.type==="gold"?0:1),gold_kudos:(target.gold_kudos||0)+(form.type==="gold"?1:0)})});
              setForm({toId:"",type:"regular",reason:""});
            }catch(e){}
          } else { send(); }
        }} disabled={sending||!form.toId||!form.reason.trim()} style={{width:"100%",padding:11}}>{sending?"Sending...":"SEND KUDO"}</SBtn></SCard>)}{tab==="pending"&&isManager&&(<div>{pending.length===0&&<SCard style={{textAlign:"center",padding:32}}><div style={{fontSize:40,marginBottom:8}}>✅</div><div style={{color:S.muted}}>No pending.</div></SCard>}{pending.map((k,i)=>{const sender=[...allStaff,...(allAgents||[])].find(u=>u.id===k.given_by);const recipient=[...allStaff,...(allAgents||[])].find(u=>u.id===k.recipient_id);const senderName=sender?.name||sender?.gameId||k.given_by||"Unknown";const recipientName=recipient?.name||recipient?.gameId||k.recipient_id||"Unknown";return(<SCard key={i} style={{marginBottom:10}}><div style={{display:"flex",gap:8,alignItems:"center",marginBottom:6}}><div style={{fontSize:24}}>{k.kudo_type==="gold"?"🌟":"👏"}</div><div style={{flex:1}}><div style={{color:S.text,fontWeight:700}}>{k.kudo_type==="gold"?"Gold Kudo":"Kudo"} · +{k.points_awarded} pts</div><div style={{color:S.accent,fontSize:12,marginTop:2}}>{senderName} <span style={{color:S.muted}}>→</span> {recipientName}</div></div></div><div style={{color:S.muted,fontSize:12,marginBottom:12,fontStyle:"italic"}}>"{k.reason}"</div><div style={{display:"flex",gap:8}}><SBtn onClick={()=>onApproveKudo(k.id,true)} color={S.green} style={{flex:1}} sm>✓ Approve</SBtn><SBtn onClick={()=>onApproveKudo(k.id,false)} color={S.red} style={{flex:1}} sm>✗ Reject</SBtn></div></SCard>);})}</div>)}</div>);}
...
export default function App(){
  ...
  const cu=loggedIn?.appType==="agents"?loggedIn:(users.find(u=>u.id===loggedIn?.id)||loggedIn);
