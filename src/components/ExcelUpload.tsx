...
export default function ExcelUpload({ onClose }: { onClose?: () => void }) {
  ...
  const processFile = async (f: File) => {
    ...
    // Check if week already loaded
    try { const ex = await sbFetch(`weekly_metrics?week=eq.${encodeURIComponent(week)}&limit=1&select=id`); setWeekAlreadyLoaded(ex?.length > 0); } catch { setWeekAlreadyLoaded(false); }

    // Get existing game_ids
    let existingIds = new Set<string>();
    try { const p = await sbFetch("profiles?select=game_id&is_active=eq.true"); existingIds = new Set((p||[]).map((x:any) => String(x.game_id).trim().toUpperCase())); } catch {}
    ...
    reader.onload = (ev) => {
      try {
        ...
        // Find metrics sheet - prefer named or second sheet
        ...
        const cCoach = col("coachid","coach_id","coachid");
        const cQcoach = col("qcoach","qacoach");
        const cAht = keys.find(k => k.toLowerCase() === "aht") || col("aht");
        const cAhtGoal = col("ahtgoal","aht_goal","ahtgoal");
        const cAhtType = col("ahttype","aht_type");
        const cQa = col("qascore","qa_score","qascore");
        const cQaGoal = col("qagoal","qa_goal","qagoal");
        ...
        const processed: ProcessedAgent[] = rows
          ...
          .map((r:any) => {
            ...
            const qaGoal = Number(r[cQaGoal]||0);
        ...
        // Parse coach sheet
        const coachName = wb.SheetNames.find(n => /coach|attrition/i.test(n)) || "";
        let parsedCoaches: CoachRow[] = [];
        if (coachName && wb.Sheets[coachName]) {
          const cr: any[] = XLSX.utils.sheet_to_json(wb.Sheets[coachName], { defval:"" });
          parsedCoaches = cr
  ...
  const handleUpload = async () => {
    ...
    if (importNew === "all") {
      for (const a of agents.filter(x=>x.is_new)) {
        try {
          await sbFetch("profiles", { method:"POST", prefer:"resolution=ignore-duplicates", body: JSON.stringify({
    ...
    for (const a of agents) {
      if (a.review_reason === "termination") {
        try { await sbFetch(`profiles?game_id=eq.${encodeURIComponent(a.game_id)}`, { method:"PATCH", body: JSON.stringify({is_active:false}) }); }
      ...
      try {
        await sbFetch("weekly_metrics", { method:"POST", prefer:"resolution=merge-duplicates", body: JSON.stringify({
    ...
    for (const c of coaches) {
      try {
        await sbFetch("staff_attrition_monthly", { method:"POST", prefer:"resolution=merge-duplicates", body: JSON.stringify({
          coach_name: c.game_id, week, voluntary_exits: c.attrition,
          position: c.position, manager_id: c.manager||null,
        })});
        if (c.manager) {
          await sbFetch(`staff_profiles?game_id=eq.${encodeURIComponent(c.game_id)}`, { method:"PATCH", body: JSON.stringify({manager_id: c.manager}) });
        }
  ...
  const S = { border:"1px solid #1e3a5f", muted:"#64748b", text:"#f1f5f9", card:"#1e293b" };
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.82)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:16}}>
      <div style={{background:"#0f172a",border:S.border,borderRadius:16,width:"100%",maxWidth:1100,maxHeight:"92vh",overflowY:"auto",padding:24,boxShadow:"0 25px 60px rgba(0,0,0,0.7)"}}>
        <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>

        {/* Header */}
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20}}>
          <div>
            <h2 style={{color:S.text,fontSize:20,fontWeight:700,margin:0}}>📊 Carga Semanal de Métricas</h2>
          ...
          <button onClick={onClose||reset} style={{background:"transparent",border:"none",color:S.muted,fontSize:20,cursor:"pointer"}}>✕</button>
        </div>
        ...
        {stage==="idle"&&(
          <div onDragOver={handleDrag} onDragLeave={handleDrag} onDrop={handleDrop}
            ...
            <p style={{color:S.text,fontSize:18,fontWeight:600,margin:"0 0 4px"}}>Arrastra tu archivo aquí</p>
            <p style={{color:S.muted,fontSize:13,margin:"0 0 20px"}}>Ej: April_12_to_April_18_KPI.xlsx</p>
            ...
            <div style={{background:"#0c2240",borderRadius:8,padding:"14px 18px",textAlign:"left",maxWidth:600,margin:"0 auto"}}>
              ...
              <p style={{color:S.muted,fontSize:12,margin:"4px 0"}}>📄 <b>Hoja de métricas</b>: Game ID · Project · Coach ID · Qcoach · AHT · AHT Goal · AHT type · QA Score · QA Goal · Absent · Tardies</p>
              <p style={{color:S.muted,fontSize:12,margin:"4px 0"}}>📄 <b>Coach Attrition</b>: Game ID · Position · Manager · Attrition</p>
            </div>
          </div>
        )}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              {[{n:agents.length,l:"Total agentes",c:"#60a5fa"},{n:ok.length,l:"Métricas OK",c:"#4ade80"},{n:flagged.length,l:"Revisión requerida",c:"#fbbf24"},{n:newA.length,l:"Agentes nuevos",c:"#a78bfa"},{n:coaches.length,l:"Coaches",c:"#f97316"}].map(c=>(
                <div key={c.l} style={{flex:1,minWidth:110,background:S.card,borderRadius:10,padding:14,textAlign:"center"}}>
                  <div style={{fontSize:24,fontWeight:700,color:c.c}}>{c.n}</div>
                  <div style={{fontSize:11,color:S.muted,marginTop:2}}>{c.l}</div>
                </div>
              ))}
            </div>
            ...
            {newA.length>0&&(
              <div style={{background:"#160d33",border:"1px solid #4c1d95",borderRadius:10,padding:16}}>
                ...
                <p style={{color:"#7c3aed",fontSize:13,margin:"0 0 12px"}}>Estos Game IDs no existen en la app. ¿Crearlos automáticamente con contraseña temporal <b style={{color:"#c4b5fd"}}>{DEFAULT_PASSWORD}</b>?</p>
                  ...
                  {flagged.map((a,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:10,background:"#0f0a00",borderRadius:8,padding:"10px 12px",flexWrap:"wrap"}}>
                      <div style={{flex:1,minWidth:180}}>
                        ...
                        <span style={{color:S.muted,fontSize:11,marginLeft:8}}>{a.project}</span>
            ...
            <div style={{overflowX:"auto",borderRadius:8,border:S.border}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                ...
                <tbody>{ok.slice(0,25).map((a,i)=>(
                  <tr key={i} style={{background:i%2===0?"#0f172a":"#0c1a2e"}}>
                    ...
                    <td style={{padding:"7px 10px",color:S.muted}}>{a.project}</td>
                    <td style={{padding:"7px 10px",color:S.muted}}>{a.coach_id}</td>
                    <td style={{padding:"7px 10px",color:"#cbd5e1"}}>{a.aht_type==="Productivity"?a.aht_seconds?.toFixed(2):a.aht_seconds?`${a.aht_seconds}s`:"-"}</td>
                    <td style={{padding:"7px 10px",color:S.muted}}>{a.aht_type==="Productivity"?a.aht_goal_seconds?.toFixed(2):a.aht_goal_seconds?`${a.aht_goal_seconds}s`:"-"}</td>
                    ...
                    <td style={{padding:"7px 10px",color:S.muted}}>{a.qa_goal>1?a.qa_goal:`${(a.qa_goal*100).toFixed(0)}%`}</td>
              ...
              {ok.length>25&&<p style={{color:"#475569",fontSize:12,padding:"8px 12px",margin:0}}>...y {ok.length-25} agentes más</p>}
            </div>
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                    ...
                    <tbody>{coaches.map((c,i)=>{
                      ...
                      return(<tr key={i} style={{background:i%2===0?"#0f172a":"#0c1a2e"}}>
                        ...
                        <td style={{padding:"7px 10px",color:S.muted}}>{c.position}</td>
        ...
        {stage==="uploading"&&(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"60px 0",gap:20}}>
            ...
            <p style={{color:S.text,fontSize:18,fontWeight:600,margin:0}}>Subiendo métricas de {weekLabel}...</p>
            ...
            <p style={{color:S.muted,fontSize:13,margin:0}}>{progress}% — {agents.length} agentes · {coaches.length} coaches</p>
          </div>
        )}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:16}}>
            ...
            <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              {[{n:summary.agents_processed,l:"Agentes procesados",c:"#60a5fa"},{n:summary.agents_created,l:"Usuarios creados",c:"#a78bfa"},{n:summary.agents_updated,l:"Métricas guardadas",c:"#4ade80"},{n:summary.coaches_processed,l:"Coaches actualizados",c:"#f97316"}].map(c=>(
                <div key={c.l} style={{flex:1,minWidth:110,background:S.card,borderRadius:10,padding:14,textAlign:"center"}}>
                  <div style={{fontSize:24,fontWeight:700,color:c.c}}>{c.n}</div>
                  <div style={{fontSize:11,color:S.muted}}>{c.l}</div>
                </div>
              ))}
            </div>
